...
const ProductDocs = () => {
  ...
  const renderArticleContent = (content: any) => {
    ...
    try {
      const contentArray = Array.isArray(content) ? content : JSON.parse(content);
      return contentArray.map((block: any, index: number) => {
        switch (block.type) {
          case 'paragraph':
            return (
              <p key={index} className="mb-4 text-foreground leading-relaxed">
                {block.content?.map((item: any, i: number) => item.text).join('')}
              </p>
            );
          case 'heading':
            const HeadingTag = `h${block.attrs?.level || 2}` as keyof JSX.IntrinsicElements;
            return (
              <HeadingTag key={index} className="font-bold mb-3 text-foreground">
                {block.content?.map((item: any, i: number) => item.text).join('')}
              </HeadingTag>
            );
          case 'bulletList':
              ...
              <ul key={index} className="list-disc pl-6 mb-4 text-foreground">
                {block.content?.map((listItem: any, i: number) => (
                  <li key={i} className="mb-1">
                    {listItem.content?.map((para: any) => 
                      para.content?.map((item: any, j: number) => item.text).join('')
                    ).join('')}
                  </li>
                ))}
              </ul>
          ...
          {selectedArticle ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Choose an article from the sidebar to view its content.</p>
              </div>
            </div>
          )}
        </div>
